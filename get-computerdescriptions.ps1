...
$computers = gc C:\oit\livermore_filtered.txt
...
$description =$OS, "$($ram.totalram)GB RAM","$($makemodel.Manufacturer +" " + $makemodel.model)", "$(" Contact: " + $userinfo.name + " Phone:" + $userinfo.telephonenumber + " Department:"+$userinfo.department)"
$separator = " | "
$data = [pscustomobject]@{ 
Computer = $Computer
...
$data | export-csv c:\oit\computerinfo-livermore.csv -append -NoTypeInformation
$data 
}
