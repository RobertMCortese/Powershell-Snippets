#Grabs a print log from a remote machine, with user that submitted the print job, date, etc.
Get-WinEvent -computername somecomputername -FilterHashTable @{LogName="Microsoft-Windows-PrintService/Operational"; ID=307; StartTime=(Get-Date -OutVariable Now).AddDays(-1)} |
